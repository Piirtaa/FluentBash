...
var stdInLines=getStdInLines();

switch(scriptArgs[0]){
	case "add":
		printGram(bookGram.addEntry(stdInLines, scriptArgs[1], scriptArgs[2]));
		break;
	case "remove":
		printGram(bookGram.removeEntry(stdInLines, scriptArgs[1], scriptArgs[2]));
		break;
	case "query":
		scriptArgs.shift();
		printGram(bookGram.query(stdInLines, ...scriptArgs));
		break;
	case "emerge":
		scriptArgs.shift();
		printGram(bookGram.emerge(stdInLines, scriptArgs[0]));
		break;
	case "getExactValues":
		scriptArgs.shift();
		printGram(bookGram.getExactValues(stdInLines, scriptArgs[0]));
		break;	
	case "format":
		scriptArgs.shift();
		print(bookGram.format(stdInLines, scriptArgs[0]));
		break;
	default:
		print("usage1:  echo $gram | ./bookGram.js add key value");
		print("  adds an entry of new data");
		print("usage2:	echo $gram | ./bookGram.js remove key value");
		print("  adds an entry to remove existing data");
		print("usage3:	echo $gram | ./bookGram.js query searchFor1 orSearchFor2");
		print("  finds entries by text match");
		print("usage4:  echo $gram | ./bookGram.js emerge pathRoot");
		print("  applies all removes and returns the summarized graph beneath the provided path.  if no path is provided the complete graph is emerged.");  
		print("usage5:  echo $gram | ./bookGram.js format formatString");
		print("  applies a format string to each line.  {0} is key.  {1} is value.");
		print("usage6:  echo $gram | ./bookGram.js getExactValues path");
		print("  emerges a specific key's value");
